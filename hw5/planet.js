...
// Початковий код лишається незмінним...
...
AFRAME.registerComponent('planet', {
	...
	init: function () {
	  ...
	  // Підказка з ім'ям
	  this.el.setAttribute('tooltip', this.data.name);
	  this.el.setAttribute('event-set__enter', "scale: 1.2 1.2 1.2");
	  this.el.setAttribute('event-set__leave', "scale: 1 1 1");
	}
  });
  ...
  AFRAME.registerComponent('main', {
	...
	tick: function () {
	  ...
	  for (let i = 0; i < this.solar_system.length; i++) {
		...
		this.solar_system[i].setAttribute('position', planet_i.pos.map(x => x / 1e9).join(' '));
	  }
	}
  });
